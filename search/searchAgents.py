...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if xy in self.corners and xy not in visitedCorners:
...
def cornersHeuristic(state, problem):
    ...
    distances = [mazeDistance(position, corner, problem.startingGameState) for corner in unvisitedCorners]
...
def foodHeuristic(state, problem):
    ...
    if len(foodGridPositions) == 0:
        return 0
    distances = [mazeDistance(position, food, problem.startingGameState) for food in foodGridPositions]
    return max(distances)
