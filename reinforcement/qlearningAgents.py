...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        legalActions = self.getLegalActions(state)
        if legalActions:
            return max(legalActions, key=lambda action: self.qvalue[(state, action)])
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.qvalue[(state, action)] += self.alpha * (reward + self.discount * self.getValue(nextState) - self.qvalue[(state, action)])
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        next_Q_values = [self.getQValue(nextState, legal_action) for legal_action in self.getLegalActions(nextState)]
